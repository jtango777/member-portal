...
type Face = { id: string; source: 'profile' | 'pending' | 'directory'; name: string }
...
// Same visual pattern as components/admin/ArchiveMemberDialog.tsx (the
// Members page's version) — built 2026-09-10 to replace the small inline
// card that used to pop up right on the photo tile. That worked fine back
// when Faces archiving was usually just "clear a bad photo," but now that
// it does the same real thing as the Members page (blocks login, syncs
// Pipedrive), looking completely different was confusing — Caroline
// caught this right after the archive behavior itself got fixed.
export default function ArchiveFaceDialog({ face, onOpenChange, onSuccess }: Props) {
  const [unmarkInPipedrive, setUnmarkInPipedrive] = useState(true)
  const [archiving, setArchiving] = useState(false)

  const copy = !face
    ? null
    : face.source === 'directory'
    ? {
        title: 'Archive photo?',
        detail: 'No account exists here — this just removes the photo, nothing else.',
        confirmLabel: 'Archive',
      }
    : face.source === 'pending'
    ? {
        title: `Archive ${face.name}'s invite?`,
        detail: 'Removes them from Members. Their invite link stops working — they can be re-invited later.',
        confirmLabel: 'Archive',
      }
    : {
        title: `Archive ${face.name}'s account?`,
        detail: 'Blocks their login and removes them from Members, Faces & reports. Can be restored later.',
        confirmLabel: 'Archive',
      }

  // Directory photos have no account/invite tied to them at all, so there's
  // nothing for Pipedrive to know about either.
  const showPipedriveToggle = face?.source === 'profile' || face?.source === 'pending'

  async function handleArchive() {
    if (!face) return
    setArchiving(true)
    const res = await fetch(`/api/admin/faces/${face.id}?source=${face.source}`, {
      method: 'DELETE',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ unmarkInPipedrive: showPipedriveToggle ? unmarkInPipedrive : false }),
    })
    if (res.ok) {
      ...
      <Dialog.Portal>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          <Dialog.Content className="bg-white rounded-xl border border-gray-200 p-6 w-full max-w-sm transition-all duration-200 data-[state=open]:opacity-100 data-[state=open]:scale-100 data-[state=closed]:opacity-0 data-[state=closed]:scale-95">
            ...
            <p className="text-xs text-gray-500 mb-6 leading-relaxed">{copy?.detail}</p>
            ...
            <div className="flex justify-end gap-2">
              ...
              <button onClick={handleArchive} disabled={archiving}
                ...
                {archiving ? 'Archiving…' : copy?.confirmLabel}
              </button>
            </div>
          </Dialog.Content>
        </div>
