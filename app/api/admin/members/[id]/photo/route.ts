...
// Upload a new photo on a member's behalf
export async function POST(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const { error: profileErr } = await admin.from('profiles').update({ avatar_url: avatarUrl }).eq('id', id)
  if (profileErr) return NextResponse.json({ error: profileErr.message }, { status: 500 })

  return NextResponse.json({ avatar_url: avatarUrl })
}

// Link an existing directory photo to a member's account
export async function PATCH(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const { error: profileErr } = await admin.from('profiles').update({ avatar_url: photo.avatar_url }).eq('id', id)
  if (profileErr) return NextResponse.json({ error: profileErr.message }, { status: 500 })

  // Remove the static entry now that it's linked to a real account, so it doesn't
  // show up twice in the Member Directory.
